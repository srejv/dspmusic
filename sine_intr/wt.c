...
fixedp SinTable[N];
fixedp SquareTable[N];
fixedp SawtoothTable[N];
fixedp TriangleTable[N];
...
void WaveTable_generateTables() {
	...
	qN = short2q(1024);					// 1024 in q (<< 16)

	oneOverN = short2q(1024);			// N (we want 1/1024)
...
void WaveTable_cookFrequency(WaveTable *this, fixedp frequency) {
	this->mInc = qmul( short2q(N), qmul(frequency, oneOverSR));
	printf("Inc: %ld \n", this->mInc);
}
